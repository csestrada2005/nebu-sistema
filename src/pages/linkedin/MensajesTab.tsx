...
const MensajesTab = () => {
  ...
  const templates = lang === "es"
    ? ["Primer contacto", "Follow-up", "Agendar llamada", "Propuesta"]
    : ["First contact", "Follow-up", "Schedule call", "Proposal"];

  return (
    <div className="rounded-lg flex flex-col h-[calc(100vh-220px)]" style={{ backgroundColor: "var(--nebu-card)", border: "1px solid var(--nebu-border)" }}>
      <div className="flex-1 overflow-y-auto p-5 space-y-3">
        {messages.map((m, i) => {
          const isJosep = m.from === "Josep";
          return (
            <div key={i} className={`flex ${isJosep ? "justify-start" : "justify-end"}`}>
              <div className="max-w-[70%] rounded-lg px-4 py-3 text-sm" style={{ backgroundColor: isJosep ? "var(--nebu-bg)" : "var(--nebu-active-bg)", color: "var(--nebu-text)", border: "1px solid var(--nebu-border)" }}>
      ...
      <div className="px-5 pb-2 flex gap-2 flex-wrap">
        {templates.map((t) => (
          <button key={t} className="text-[11px] px-3 py-1 rounded-full font-medium transition-colors"
            style={{ backgroundColor: "var(--nebu-active-bg)", color: "var(--nebu-text-secondary)", border: "1px solid var(--nebu-border)" }}>{t}</button>
        ))}
      </div>
      <div className="p-4 flex gap-2" style={{ borderTop: "1px solid var(--nebu-border)" }}>
        <input value={input} onChange={(e) => setInput(e.target.value)}
          ...
          className="flex-1 text-sm rounded-md px-3 py-2" style={{ backgroundColor: "var(--nebu-bg)", color: "var(--nebu-text)", border: "1px solid var(--nebu-border)" }} />
        <button className="px-4 py-2 rounded-md text-sm font-semibold flex items-center gap-1.5" style={{ backgroundColor: "var(--nebu-accent)", color: "#fff" }}>
