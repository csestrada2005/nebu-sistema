...
const sequences: Sequence[] = [
  { id: 1, name: "Prospección Fría — Sin Web", status: "activa", desc: "Para negocios sin presencia digital detectados en LinkedIn", contacts: 34, openRate: "42%", replyRate: "14%", steps: [
  ...
  { id: 3, name: "Reactivación Clientes Pasados", status: "pausada", desc: "Para clientes con proyectos terminados hace 3+ meses", contacts: 8, openRate: "55%", replyRate: "18%", steps: [
    { day: 0, type: "email" }, { day: 14, type: "email" }, { day: 30, type: "email" },
  ]},
  { id: 4, name: "Web Vieja — Oportunidad CRO", status: "borrador", desc: "Para negocios con web desactualizada o de baja conversión", contacts: 0, openRate: "-", replyRate: "-", steps: [
...
const contacts: Contact[] = [
  { name: "Carlos M.", company: "Clínica Sonrisa", industry: "Salud", sequence: "Prospección Fría", step: "Paso 2/4", status: "sin_respuesta", lastEmail: "Hace 3 días" },
  { name: "Ana R.", company: "La Hacienda", industry: "Restaurante", sequence: "Prospección Fría", step: "Paso 1/4", status: "abrio", lastEmail: "Hace 1 día" },
  { name: "Roberto S.", company: "Constructora SB", industry: "Construcción", sequence: "Post-Llamada", step: "Paso 2/4", status: "respondio", lastEmail: "Ayer" },
  { name: "Miguel T.", company: "Despacho MTL", industry: "Legal", sequence: "Prospección Fría", step: "Paso 3/4", status: "sin_respuesta", lastEmail: "Hace 7 días" },
  { name: "Laura V.", company: "Boutique Flores", industry: "Retail", sequence: "Web Vieja", step: "-", status: "sin_respuesta", lastEmail: "-" },
  { name: "Diana K.", company: "Coach Negocios", industry: "Servicios", sequence: "Post-Llamada", step: "Paso 1/4", status: "abrio", lastEmail: "Hace 2 días" },
  { name: "Jorge P.", company: "Inmobiliaria JP", industry: "Inmuebles", sequence: "Reactivación", step: "Paso 2/3", status: "respondio", lastEmail: "Hace 4 días" },
  { name: "Sandra L.", company: "Spa Serenity", industry: "Salud/Bienestar", sequence: "Prospección Fría", step: "Paso 1/4", status: "sin_respuesta", lastEmail: "Hace 1 día" },
];
...
const templates: Template[] = [
  { id: 1, name: "Prospección Sin Web", category: "Prospección", categoryColor: "#3B82F6", subject: "{{negocio}} — vi que no tienen web", preview: "Hola {{nombre}}, encontré tu negocio buscando {{industria}} en México y noté que no tienen presencia web profesional...", variables: ["nombre", "negocio", "industria", "senal_dolor"], usedIn: 2, openRate: "44%" },
  { id: 2, name: "Follow-up Día 3", category: "Follow-up", categoryColor: "#EAB308", subject: "¿Lo viste, {{nombre}}?", preview: "Solo quería retomar mi mensaje anterior. Sé que eres...", variables: ["nombre"], usedIn: 3, openRate: "38%" },
...
const statusBadge = (s: Sequence["status"]) => {
  ...
  const c = m[s];
  return <span style={{ backgroundColor: c.bg, color: c.text, fontSize: 11, fontWeight: 600, padding: "2px 10px", borderRadius: 9999 }}>{c.label}</span>;
};

const contactStatusBadge = (s: Contact["status"]) => {
  const m: Record<string, { bg: string; text: string; label: string }> = {
    sin_respuesta: { bg: "#6B728022", text: "#9CA3AF", label: "Sin respuesta" },
    abrio: { bg: "#EAB30822", text: "#EAB308", label: "Abrió ✓" },
    respondio: { bg: "#22C55E22", text: "#22C55E", label: "Respondió ✓✓" },
    reboto: { bg: "#EF444422", text: "#EF4444", label: "Rebotó ✗" },
  };
  const c = m[s];
  return <span style={{ backgroundColor: c.bg, color: c.text, fontSize: 11, fontWeight: 600, padding: "2px 10px", borderRadius: 9999 }}>{c.label}</span>;
};
    ...
    {steps.map((s, i) => (
      <div key={i} style={{ display: "flex", alignItems: "center" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          ...
          <span style={{ fontSize: 9, color: "#666" }}>Día {s.day}</span>
        </div>
      ...
      {sequences.map((seq) => (
        <div key={seq.id} style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 20, border: "1px solid #262626" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
            <div>
              <span style={{ fontWeight: 700, color: "#fff", fontSize: 15 }}>{seq.name}</span>
              <div style={{ marginTop: 4 }}>{statusBadge(seq.status)}</div>
            </div>
          </div>
          <p style={{ color: "#999", fontSize: 13, marginBottom: 12 }}>{seq.desc}</p>
          <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#aaa", marginBottom: 4 }}>
            <span>{seq.contacts} contactos</span>
            <span>{seq.openRate} apertura</span>
            <span>{seq.replyRate} respuesta</span>
          </div>
          <MiniTimeline steps={seq.steps} status={seq.status} />
          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <button onClick={() => setExpanded(expanded === seq.id ? null : seq.id)} style={{ padding: "4px 14px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 12, cursor: "pointer" }}>Ver</button>
            <button style={{ padding: "4px 14px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 12, cursor: "pointer" }}>{seq.status === "activa" ? "Pausar" : "Activar"}</button>
            <button style={{ padding: "4px 14px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 12, cursor: "pointer" }}>Duplicar</button>
          </div>
          {expanded === seq.id && seq.steps.map((step, i) => (
            <div key={i} style={{ backgroundColor: "#111", borderRadius: 8, padding: 12, marginTop: 8, borderLeft: "3px solid #E53E3E" }}>
              <div style={{ fontWeight: 600, color: "#ddd", fontSize: 13 }}>
                {step.type === "email" ? `Email ${i + 1} (Día ${step.day})` : `Llamada (Día ${step.day})`}
              </div>
              {step.subject && <div style={{ color: "#aaa", fontSize: 12, marginTop: 2 }}>Asunto: {step.subject}</div>}
              {step.preview && <div style={{ color: "#777", fontSize: 12, marginTop: 2, fontStyle: "italic" }}>{step.preview}</div>}
              {step.label && <div style={{ color: "#777", fontSize: 12, marginTop: 2 }}>{step.label}</div>}
            </div>
          ))}
        </div>
      ))}
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <select style={{ backgroundColor: "#111", border: "1px solid #333", borderRadius: 8, padding: "8px 12px", color: "#aaa", fontSize: 13 }}><option>Secuencia</option></select>
        <select style={{ backgroundColor: "#111", border: "1px solid #333", borderRadius: 8, padding: "8px 12px", color: "#aaa", fontSize: 13 }}><option>Estado</option></select>
        <select style={{ backgroundColor: "#111", border: "1px solid #333", borderRadius: 8, padding: "8px 12px", color: "#aaa", fontSize: 13 }}><option>Industria</option></select>
      </div>
      <div style={{ backgroundColor: "#1a1a1a", borderRadius: 12, border: "1px solid #262626", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #262626" }}>
              {["Nombre", "Empresa", "Industria", "Secuencia", "Paso actual", "Estado", "Último email"].map((h) => (
                <th key={h} style={{ padding: "10px 14px", fontSize: 11, fontWeight: 600, color: "#666", textAlign: "left", textTransform: "uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {contacts.map((c, i) => (
              <tr key={i} onClick={() => setSelectedContact(c)} style={{ borderBottom: "1px solid #1f1f1f", cursor: "pointer" }} onMouseOver={(e) => (e.currentTarget.style.backgroundColor = "#222")} onMouseOut={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                ...
                <td style={{ padding: "10px 14px" }}>{contactStatusBadge(c.status)}</td>
                <td style={{ padding: "10px 14px", color: "#666", fontSize: 12 }}>{c.lastEmail}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ position: "fixed", top: 0, right: 0, width: 400, height: "100vh", backgroundColor: "#111", borderLeft: "1px solid #262626", zIndex: 1000, padding: 24, overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ backgroundColor: "#1a1a1a", borderRadius: 8, padding: 14 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Empresa</div>
              <div style={{ color: "#fff", fontSize: 14 }}>{selectedContact.company}</div>
            </div>
            <div style={{ backgroundColor: "#1a1a1a", borderRadius: 8, padding: 14 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Industria</div>
              <div style={{ color: "#fff", fontSize: 14 }}>{selectedContact.industry}</div>
            </div>
            <div style={{ backgroundColor: "#1a1a1a", borderRadius: 8, padding: 14 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Secuencia</div>
              <div style={{ color: "#fff", fontSize: 14 }}>{selectedContact.sequence} — {selectedContact.step}</div>
            </div>
            <div style={{ backgroundColor: "#1a1a1a", borderRadius: 8, padding: 14 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Estado</div>
              <div>{contactStatusBadge(selectedContact.status)}</div>
            </div>
            <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginTop: 8 }}>Timeline de emails</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {["📧 Enviado", "👁️ Abierto", "↩️ Respondido"].map((label, i) => (
                <span key={i} style={{ backgroundColor: "#1a1a1a", padding: "4px 10px", borderRadius: 6, fontSize: 11, color: "#aaa" }}>{label}</span>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
              <button style={{ padding: "6px 16px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 12, cursor: "pointer" }}>Pausar</button>
              <button style={{ padding: "6px 16px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 12, cursor: "pointer" }}>Cambiar secuencia</button>
              <button style={{ padding: "6px 16px", borderRadius: 6, border: "1px solid #E53E3E", backgroundColor: "transparent", color: "#E53E3E", fontSize: 12, cursor: "pointer" }}>Mover a CRM</button>
              <button style={{ padding: "6px 16px", borderRadius: 6, border: "1px solid #22C55E", backgroundColor: "transparent", color: "#22C55E", fontSize: 12, cursor: "pointer" }}>Marcar respondido</button>
            </div>
          </div>
        </div>
      )}
...
const PlantillasTab = () => {
  ...
  const cats = ["Todas", "Prospección", "Follow-up", "Nurturing", "Cierre"];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        {filtered.map((t) => (
          <div key={t.id} style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 18, border: "1px solid #262626" }}>
            ...
            <div style={{ color: "#aaa", fontSize: 12, marginBottom: 4 }}>Asunto: <HighlightVars text={t.subject} /></div>
            ...
            <div style={{ color: "#666", fontSize: 11, marginBottom: 10 }}>Usada en {t.usedIn} secuencias | Apertura: {t.openRate}</div>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={() => setViewTemplate(t)} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 11, cursor: "pointer" }}>Ver completa</button>
              <button style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 11, cursor: "pointer" }}>Usar</button>
              <button style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 11, cursor: "pointer" }}>Editar</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setViewTemplate(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: "#1a1a1a", borderRadius: 16, padding: 28, width: 560, maxHeight: "80vh", overflowY: "auto", border: "1px solid #333" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Asunto</div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Cuerpo</div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 6 }}>Variables detectadas</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setViewTemplate(null)} style={{ padding: "6px 18px", borderRadius: 8, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 13, cursor: "pointer" }}>Cerrar</button>
              <button style={{ padding: "6px 18px", borderRadius: 8, border: "1px solid #333", backgroundColor: "transparent", color: "#ccc", fontSize: 13, cursor: "pointer" }}>Duplicar</button>
              <button style={{ padding: "6px 18px", borderRadius: 8, border: "none", backgroundColor: "#E53E3E", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Usar en secuencia</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 18, border: "1px solid #262626" }}>
          ...
          <span style={{ color: "#22C55E", fontSize: 12 }}>↑ Tendencia positiva</span>
        </div>
        <div style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 18, border: "1px solid #262626" }}>
          <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 6 }}>Tasa de apertura</div>
          <div style={{ color: "#fff", fontSize: 28, fontWeight: 700 }}>38%</div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ color: "#999", fontSize: 11 }}>Benchmark: 25%</span>
            <span style={{ backgroundColor: "#22C55E22", color: "#22C55E", fontSize: 10, padding: "2px 8px", borderRadius: 9999, fontWeight: 600 }}>✓ Sobre promedio</span>
          </div>
        </div>
        <div style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 18, border: "1px solid #262626" }}>
          <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 6 }}>Tasa de respuesta</div>
        ...
        <div style={{ backgroundColor: "#1a1a1a", borderRadius: 12, padding: 18, border: "1px solid #262626" }}>
          <div style={{ color: "#666", fontSize: 11, textTransform: "uppercase", marginBottom: 6 }}>Reuniones generadas</div>
          <div style={{ color: "#fff", fontSize: 28, fontWeight: 700 }}>4</div>
          <span style={{ backgroundColor: "#E53E3E22", color: "#E53E3E", fontSize: 10, padding: "2px 8px", borderRadius: 9999, fontWeight: 600 }}>⚡ Este mes</span>
        </div>
      </div>
